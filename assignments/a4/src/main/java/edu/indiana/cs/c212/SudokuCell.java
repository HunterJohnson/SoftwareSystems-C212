...
class SudokuCell {
	...
	public Boolean isLegalToInsert(int value, SudokuPuzzle currentPuzzle) {
		...
		//FIXME
		if(getRow() == -1 || getColumn() == -1)      //bounds check
			return false;
		if(value > rowCount || value < 0)
			return false;
		
		if(isLegalForRow(value) && isLegalForColumn(value) && isLegalForBox(value))
			return true;
	...
	private Boolean isLegalForRow(int digit) {
		//FIXME
		for(int c=getColumn(); c < columnCount; c++)
				 if(puzzle.get(getRow(), c) == digit)
					 return false;		
		return true;
	}
	...
	private Boolean isLegalForColumn(int digit) {
		//FIXME
		for(int r=getRow(); r < rowCount; r++)
			if(puzzle.get(r,getColumn()) == digit)
				return false;
		return true;
	}
